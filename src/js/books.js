...
if (document.querySelector('.books')) {
    if (JSON.parse(localStorage.getItem('token'))) {
        ...
        const warningUpdate = document.querySelectorAll('.update-item__warning')
        ...
        const getBooks = async () => {
            ...
            response.map(item => {
                const template = `
            <div data-index="${item.id}" class="books-item">
                <div data-index="${item.id}">
                    <p data-index="${item.id}">${item.name}</p>
                    <h3 data-index="${item.id}">${item.author}</h3>
        ...
        const openDetails = async () => {
            const details = document.querySelectorAll('.books-item')
            details.forEach(item => {
                item.addEventListener('click', async (e) => {
                    ...
                    const book = await getBooksItemFromAPI(id)
                    const template = `
                            <div>
                               <p><span>Название книги:</span> ${book.name}.</p>
                            </div>
                            <div>
                               <p><span>Автор книги:</span> ${book.author}.</p>
                            </div>
                            <div>
                                <p><span>Жанры:</span> ${book.genres}.</p> 
                            </div>
                            <div>
                                <p><span>Кол-во страниц:</span> ${book.pagesNumber} стр.</p>
                            </div>
                            <div>
                                <p><span>Издательство:</span>  ${book.publishHouse}. </p> 
                            </div>
                            <div>
                                <p><span>Год издания:</span>  ${book.publishYear} г.</p>
                            </div>
                        `
                    detailWrapper.insertAdjacentHTML('afterbegin', template)
                })
            })
        }
        ...
        const validateUpdate = () => {
            for (let i = 0; i <= updateItem.length - 1; i++) {
                if (updateItem[i].value.trim().length === 0) {
                    warningUpdate[i].style.opacity = 1;
                    warningUpdate[i].style.visibility = 'visible';

                } else {
                    warningUpdate[i].style.opacity = 0;
                    warningUpdate[i].style.visibility = 'hidden';
                }
            }
        }
